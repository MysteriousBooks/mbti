...
function generateStructuralPath(element: HTMLElement, variantRoot: HTMLElement): string {
  ...
  while (current && current !== variantRoot && current.parentElement) {
    ...
    const siblings: HTMLElement[] = Array.from(parent.children) as HTMLElement[];
...
function calculateCoordinates(variantRoot: HTMLElement, clickX: number, clickY: number): Coordinates {
...
function identifyElement(
  ...
  return {
    ...
    coordinates: calculateCoordinates(variantRoot, clickX, clickY),
  };
}
...
export function FeedbackOverlay({
  ...
  const handleDocumentClick = useCallback(
    (e: MouseEvent) => {
      ...
      (target as unknown as { __feedbackClickInfo?: unknown }).__feedbackClickInfo = {
        variantInfo,
        clickX: e.clientX,
        clickY: e.clientY,
      };
  ...
  const saveComment = () => {
    ...
    const clickInfo = (state.selectedElement as unknown as { __feedbackClickInfo?: { variantInfo: { root: HTMLElement; variantId: VariantId }; clickX: number; clickY: number } }).__feedbackClickInfo;
    if (!clickInfo) return;
    ...
    delete (state.selectedElement as unknown as { __feedbackClickInfo?: unknown }).__feedbackClickInfo;
  };
